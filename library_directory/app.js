...
libApp.controller('mainDirectory', ['$scope', function($scope){
	...
	$scope.navigation = [
		...
		,{
			...
			name : 'underscore'
			,pre : '<script src="https://cdnjs.cloudflare.com/ajax/libs/underscore.js/'
			,close : '/underscore-min.js"></script>'
		}
		/*,{
			name : 'requests'
	...
	$scope.$watch("currentDir", function(value){
			...
			if (value.name === 'jquery'){
				...
				$scope.jqueryBool = false;
				$scope.underscoreBool = false;
			} else if (value.name === 'underscore'){
				$scope.underscoreBool = true;
				$scope.jqueryBool = false;
				$scope.angularBool = false;
			}
		}
